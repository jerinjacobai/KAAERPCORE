-- ==============================================================================
-- KAA ERP V1.1 - MASTER DATA SEED SCRIPT (SQL Editor Safe Version)
-- This version automatically gets your company_id from the database
-- ==============================================================================

-- This script will use the first company_id found in your profiles/companies table
-- It's safe to run directly in Supabase SQL Editor

DO $$
DECLARE
    v_company_id UUID;
BEGIN
    -- Get the first company_id from profiles table
    SELECT company_id INTO v_company_id FROM profiles LIMIT 1;
    
    -- If no company found, raise error
    IF v_company_id IS NULL THEN
        RAISE EXCEPTION 'No company_id found in profiles table. Please create a user/company first.';
    END IF;
